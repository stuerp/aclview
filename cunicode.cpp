...
WCHAR * wcslcpy(WCHAR * str1, const WCHAR * str2, int imaxlen)
{
    ...
    {
        ::wcsncpy(str1, str2, (size_t)(imaxlen - 1));
        str1[imaxlen - 1] = 0;
    }
    else
        wcscpy(str1, str2);
...
WCHAR * wcslcat(wchar_t * str1, const WCHAR * str2, int imaxlen)
{
    ...
    {
        ::wcsncpy(str1 + l1, str2, (size_t)(imaxlen - 1 - l1));
        str1[imaxlen - 1] = 0;
    }
    else
        wcscat(str1, str2);
...
{
    awlcopy(lpFindFileDataW->cAlternateFileName, lpFindFileDataA->cAlternateFileName, _countof(lpFindFileDataW->cAlternateFileName) - 1);
    awlcopy(lpFindFileDataW->cFileName, lpFindFileDataA->cFileName, _countof(lpFindFileDataW->cFileName) - 1);
...
HANDLE FindFirstFileT(WCHAR * lpFileName, LPWIN32_FIND_DATAW lpFindFileData)
{
    ...
    else
    {
        ...
        {
            awlcopy(lpFindFileData->cAlternateFileName, FindFileDataA.cAlternateFileName, _countof(lpFindFileData->cAlternateFileName) - 1);
            awlcopy(lpFindFileData->cFileName, FindFileDataA.cFileName, _countof(lpFindFileData->cFileName) - 1);
